#!/usr/bin/env bash
#for dev-release apk
LATEST_APK_DEV=$(ls -lrt ./Presentation/build/outputs/apk/dev/release/*.apk | tail -1 | awk -F" " '{ print $9 }') #Pick the latest dev build apk.
...
LATEST_APK_STAGING=$(ls -lrt ./Presentation/build/outputs/apk/staging/release/*.apk | tail -1 | awk -F" " '{ print $9 }') #Pick the latest Staging build apk.
...
LATEST_APK_PRODUCTION=$(ls -lrt ./Presentation/build/outputs/apk/production/release/*.apk | tail -1 | awk -F" " '{ print $9 }') #Pick the latest production build apk.
